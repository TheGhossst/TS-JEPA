"""Plan §12 exact TS-JEPA training hyperparameters."""
...
# Plan §12 explicitly forbids generic BYOL/JEPA draft defaults.
...
def assert_plan_jepa_training_config(config: dict[str, Any]) -> None:
    """
    Raise ValueError when JEPA training hyperparameters deviate from plan §12.
    ...
    if opt_type in FORBIDDEN_JEPA_TRAINING_DEFAULTS["optimizer_types"]:
        errors.append(
            f"ts_jepa.optimizer.type {opt_type!r} is a forbidden BYOL/JEPA draft default (plan §12)"
        )
    ...
    if abs(ema - FORBIDDEN_JEPA_TRAINING_DEFAULTS["ema_decay"]) < 1e-12:
        errors.append("ts_jepa.target_encoder.ema_decay 0.996 is forbidden (plan §12)")
    ...
    if int(enc.get("embedding_dim", -1)) != PLAN_JEPA_TRAINING["embedding_dim"]:
        errors.append(f"ts_jepa.encoder.embedding_dim: expected 256, got {enc.get('embedding_dim')}")

    if errors:
        raise ValueError("Plan §12 JEPA training config mismatch:\n  - " + "\n  - ".join(errors))
