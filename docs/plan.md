...
Teacher / ground-truth control sequence used as the conditioning

sequence.



A separately trained semantic actor generates the command sequence.



A sequential actor-predictor procedure.



Another mechanism explicitly recovered from the original
...
Compute required transmission power.



Mark infeasible devices.



Compute the drift-plus-penalty scheduling score.



Ignore non-positive candidates.



Select at most J devices.



Transmit selected devices.
...
Use predicted control for devices whose state is not freshly
...
**Never replace an unspecified paper detail with a generic default

and call it paper-exact.**



**Keep paper facts and implementation choices explicitly

separated.**



**Do not add virtual inputs to the predictor unless the source

explicitly requires them.**



**Do not use random Uniform(-20,20) actions for the teacher dataset

when reproducing the current corrected setup.**



Validate TS-JEPA before implementing wireless scheduling.



**Validate the wireless baseline before introducing Gilbert-Elliott

burst losses.**



Introduce BPE and the burst-aware loss as separate ablations.



**Keep the original TS-JEPA architecture fixed while measuring the

contribution of GE-JEPA components.**



**Record every unresolved implementation choice in experiment

configuration files and experiment logs.**



**Do not report an implementation choice as an original-paper
